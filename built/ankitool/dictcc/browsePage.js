...
function tokenizeBrowsePage($) {
    ...
    var table_element = $('div[id="maincontent"]').find('dl');
    table_element.children().each(function (index, row) {
        ...
        if ($(row)[0].name == INPUT_ROW) {
            ...
            var mod = $(row).find('var').contents()
                ...
                .text().trim();
            list.push({ type: 'input_word', val: text, mod: mod });
        }
...
function parseInputWord(tokens) {
    ...
    var _a = tokens.shift(), val = _a.val, mod = _a.mod;
    data = {
        input: val,
...
function parsePageData(tokens) {
    ...
    while (tokens.length > 0) {
        if (tokens[0].type == 'input_word') {
            var input_word = parseInputWord(tokens);
...
function getTargetsDictCCBrowse(input_word, request) {
     ...
     */
    var RETRIEVAL_URL = "https://browse.dict.cc/"
        + request.input_lang
        + "-"
        + request.target_lang
    ...
    return new Promise(function (accept, reject) {
        axios.get(RETRIEVAL_URL)
            ...
            var $ = cheerio.load(data.data, null, false);
